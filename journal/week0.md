...
```json
{
    ...
    "Account": "123456789020",
    "Arn": "arn:aws:iam::123456789020:user/terraform-beginner-bootcamp"
}
```
...
| `console`      | Try Terraform expressions at an interactive prompt  |
...
When I was trying to run `terraform apply`, I encountered an error in my terraform configuration, the `random_bucket_name` was in uppercase letters, this is not acceptable for S3 Buckets. Use only lowercase alphanumeric letters and hyphens (-).
Avoid uppercase letters, underscores, or special characters to prevent errors during `terraform apply`.
